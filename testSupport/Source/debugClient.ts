...
export class DebugClient extends ProtocolClient {
	...
	): Promise<DebugProtocol.ConfigurationDoneResponse> {
	...
	): Promise<DebugProtocol.RestartResponse> {
	...
	): Promise<DebugProtocol.DataBreakpointInfoResponse> {
		return this.send("dataBreakpointInfo", args);
	}

	public setDataBreakpointsRequest(
		args: DebugProtocol.SetDataBreakpointsArguments,
	): Promise<DebugProtocol.SetDataBreakpointsResponse> {
	...
	): Promise<DebugProtocol.ModulesResponse> {
	...
	): Promise<DebugProtocol.StepInTargetsResponse> {
		...
		return new Promise((resolve, reject) => {
			...
			if (!this._socket) {
				...
				timeoutHandler = setTimeout(() => {
					reject(
						new Error(
							`no event '${eventType}' received after ${timeout} ms`,
						),
					);
				}, timeout);
			}
	...
	public assertStoppedLocation(
		reason: string,
		expected: { path?: string | RegExp; line?: number; column?: number },
	): Promise<DebugProtocol.StackTraceResponse> {
		...
		return new Promise((resolve, reject) => {
			...
			if (!this._socket) {
				...
				timeoutHandler = setTimeout(() => {
					reject(
						new Error(
							`not enough output data received after ${timeout} ms`,
						),
					);
				}, timeout);
			}
