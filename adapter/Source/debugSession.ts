	...
	implements DebugProtocol.ContinuedEvent
{
	...
	public constructor(threadId: number, allThreadsContinued?: boolean) {
		...
		if (typeof allThreadsContinued === "boolean") {
			(<DebugProtocol.ContinuedEvent>this).body.allThreadsContinued =
				allThreadsContinued;
		}
	}
}
...
export class InitializedEvent
	extends Event
	implements DebugProtocol.InitializedEvent
{
	public constructor() {
		super("initialized");
	}
...
export class BreakpointEvent
	extends Event
	implements DebugProtocol.BreakpointEvent
{
	body: {
		reason: string;
...
export class LoadedSourceEvent
	extends Event
	implements DebugProtocol.LoadedSourceEvent
{
	body: {
		reason: "new" | "changed" | "removed";
...
export class CapabilitiesEvent
	extends Event
	implements DebugProtocol.CapabilitiesEvent
{
	body: {
...
export class ProgressStartEvent
	extends Event
...
export class ProgressEndEvent
	extends Event
	implements DebugProtocol.ProgressEndEvent
{
	body: {
		progressId: string;
	};
...
export class InvalidatedEvent
	extends Event
	implements DebugProtocol.InvalidatedEvent
{
	body: {
...
export class DebugSession extends ProtocolServer {
	...
	public constructor(
		...
		this._isServer =
			typeof obsolete_isServer === "boolean" ? obsolete_isServer : false;
	...
	public runInTerminalRequest(
		...
		this.sendRequest(
			...
			args,
			timeout,
			cb as (r: DebugProtocol.Response) => void,
		);
	}
	...
	protected dispatchRequest(request: DebugProtocol.Request): void {
		...
		try {
			if (request.command === "initialize") {
				var args = <DebugProtocol.InitializeRequestArguments>(
					request.arguments
				);
				...
				this.launchRequest(
					<DebugProtocol.LaunchResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "attach") {
				this.attachRequest(
					<DebugProtocol.AttachResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "disconnect") {
				this.disconnectRequest(
					<DebugProtocol.DisconnectResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "terminate") {
				this.terminateRequest(
					<DebugProtocol.TerminateResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "restart") {
				this.restartRequest(
					<DebugProtocol.RestartResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "setBreakpoints") {
				this.setBreakPointsRequest(
					<DebugProtocol.SetBreakpointsResponse>response,
					request.arguments,
					request,
				);
				...
				this.configurationDoneRequest(
					<DebugProtocol.ConfigurationDoneResponse>response,
					request.arguments,
					request,
				);
				...
				this.nextRequest(
					<DebugProtocol.NextResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "stepIn") {
				this.stepInRequest(
					<DebugProtocol.StepInResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "stepOut") {
				this.stepOutRequest(
					<DebugProtocol.StepOutResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "stepBack") {
				this.stepBackRequest(
					<DebugProtocol.StepBackResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "reverseContinue") {
				this.reverseContinueRequest(
					<DebugProtocol.ReverseContinueResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "restartFrame") {
				this.restartFrameRequest(
					<DebugProtocol.RestartFrameResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "goto") {
				this.gotoRequest(
					<DebugProtocol.GotoResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "pause") {
				this.pauseRequest(
					<DebugProtocol.PauseResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "stackTrace") {
				this.stackTraceRequest(
					<DebugProtocol.StackTraceResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "scopes") {
				this.scopesRequest(
					<DebugProtocol.ScopesResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "variables") {
				this.variablesRequest(
					<DebugProtocol.VariablesResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "setVariable") {
				this.setVariableRequest(
					<DebugProtocol.SetVariableResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "setExpression") {
				this.setExpressionRequest(
					<DebugProtocol.SetExpressionResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "source") {
				this.sourceRequest(
					<DebugProtocol.SourceResponse>response,
					request.arguments,
					request,
				);
				...
				this.terminateThreadsRequest(
					<DebugProtocol.TerminateThreadsResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "evaluate") {
				this.evaluateRequest(
					<DebugProtocol.EvaluateResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "stepInTargets") {
				this.stepInTargetsRequest(
					<DebugProtocol.StepInTargetsResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "gotoTargets") {
				this.gotoTargetsRequest(
					<DebugProtocol.GotoTargetsResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "completions") {
				this.completionsRequest(
					<DebugProtocol.CompletionsResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "exceptionInfo") {
				this.exceptionInfoRequest(
					<DebugProtocol.ExceptionInfoResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "loadedSources") {
				this.loadedSourcesRequest(
					<DebugProtocol.LoadedSourcesResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "dataBreakpointInfo") {
				this.dataBreakpointInfoRequest(
					<DebugProtocol.DataBreakpointInfoResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "setDataBreakpoints") {
				this.setDataBreakpointsRequest(
					<DebugProtocol.SetDataBreakpointsResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "readMemory") {
				this.readMemoryRequest(
					<DebugProtocol.ReadMemoryResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "writeMemory") {
				this.writeMemoryRequest(
					<DebugProtocol.WriteMemoryResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "disassemble") {
				this.disassembleRequest(
					<DebugProtocol.DisassembleResponse>response,
					request.arguments,
					request,
				);
			} else if (request.command === "cancel") {
				this.cancelRequest(
					<DebugProtocol.CancelResponse>response,
					request.arguments,
					request,
				);
				...
				this.setInstructionBreakpointsRequest(
					<DebugProtocol.SetInstructionBreakpointsResponse>response,
					request.arguments,
					request,
				);
			} else {
				this.customRequest(
					request.command,
					<DebugProtocol.Response>response,
					request.arguments,
					request,
				);
			}
